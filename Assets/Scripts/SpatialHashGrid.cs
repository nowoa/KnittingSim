...
public class SpatialHashGrid
{
    ...
    public static Dictionary<Vector2Int,List<int>> PartitionScreen<T>(IList<T> items, Func<T, Vector3> positionGetter, float cellSize = 5f)
    {
        Dictionary<Vector2Int, List<int>> result = new Dictionary<Vector2Int, List<int>>();
        for(int i = 0; i < items.Count; i++)
        {
            Vector2 pos = positionGetter(items[i]);
            Vector2Int cellKey = GetCellKey2D(pos,cellSize);
            if (!result.ContainsKey(cellKey))
            {
                result.Add(cellKey,new List<int>());
            }
